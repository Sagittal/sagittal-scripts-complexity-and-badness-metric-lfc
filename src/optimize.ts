...
const logComplexityParameterSetsForComplexityMetricFamilyWhichOptimizeItsGrade = ([
    complexityMetricFamilyId,
    { metric, parameters },
]: [ComplexityMetricFamilyId, { metric: ComplexityMetric; parameters: ComplexityParameterId[] }]): void => {
    ...
    complexityParameterSets.forEach((complexityParameterSet: ComplexityParameterSet, index: number): void => {
        ...
        saveLog(
            `Complexity parameter set (${index + 1}/${countComplexityParameterSetsForComplexityMetricFamily}): ${stringify(complexityParameterSet)} -> metric grade ${metricGrade}`,
            LogTarget.PROGRESS,
        )
    })

    saveLog(
        `Complexity parameter sets for complexity metric family ${complexityMetricFamilyId} which optimize its metric grade, all bringing it to ${minMetricGrade} (count of ties ${count(complexityParameterSetsForComplexityMetricFamilyWhichOptimizeItsGrade)}): ${stringify(complexityParameterSetsForComplexityMetricFamilyWhichOptimizeItsGrade)}`,
        LogTarget.FINAL,
    )
}

export { logComplexityParameterSetsForComplexityMetricFamilyWhichOptimizeItsGrade }
