...
import * as save from "@sagittal/general/dist/cjs/node/scripts/save"
...
describe("logComplexityParameterSetsForComplexityMetricFamilyWhichOptimizeItsGrade", (): void => {
    it("computes the parameter sets for the given metric family, then checks each one, and finds which one minimizes its grade", (): void => {
        ...
        const metricFamilyWithParameters =
            COMPLEXITY_METRIC_FAMILIES_WITH_PARAMETERS[complexityMetricFamilyId]
        ...
        expect(save.saveLog).toHaveBeenCalledWith(
            `Complexity parameter set (1/2): {"sE":3,"c":5.4,"tP":5} -> metric grade 7`,
            LogTarget.PROGRESS,
        )
        expect(save.saveLog).toHaveBeenCalledWith(
            `Complexity parameter set (2/2): {"sE":1.3,"c":1,"tP":-0.1} -> metric grade 4`,
            LogTarget.PROGRESS,
        )
        expect(save.saveLog).toHaveBeenCalledWith(
            `Complexity parameter sets for complexity metric family lep which optimize its metric grade, all bringing it to 4 (count of ties 1): [{"sE":1.3,"c":1,"tP":-0.1}]`,
            LogTarget.FINAL,
        )
    })
})
